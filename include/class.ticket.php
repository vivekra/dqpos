...
include_once(INCLUDE_DIR.'class.serial.php');
...
class Ticket {
    ...
    function load($id=0) {
        ...
        $sql='SELECT  ticket.*, slno.serial_id, slno.serial_no, slno.model_no, slno.manufacturer ' 
			.' ,slno.invoice_date, slno.invoice_no, slno.billing_name, slno.billing_address '
			.' ,slno.billing_location, lock_id, dept_name '
            .' ,IF(sla.id IS NULL, NULL, '
                .'DATE_ADD(ticket.created, INTERVAL sla.grace_period HOUR)) as sla_duedate '
            .' ,count(distinct attach.attach_id) as attachments'
            .' FROM '.TICKET_TABLE.' ticket '
            .' LEFT JOIN '.DEPT_TABLE.' dept ON (ticket.dept_id=dept.dept_id) '
			.' LEFT JOIN '.SERIAL_TABLE.' slno ON (ticket.serial_id=slno.serial_id) '
	...
	function getMobileNumber() {
        return (string)$this->getOwner()->getMobileNumber();
    }
	...
	function getNatureCall() {
        return $this->ht['nature_call'];
    }
	
	function getCustRefNo() {
        return $this->ht['customer_ticket_id'];
    }
	
	function getVendorRefNo() {
        return $this->ht['vendor_ticket_id'];
    }
	
	
	function getCallLoggedIn() {
        return $this->ht['call_logged_in'];
    }
   ...
   function onOverdue($whine=true, $comments="") {
    ...
    function update($vars, &$errors) {
        ...
        $fields=array();
        $fields['topicId']  = array('type'=>'int',      'required'=>1, 'error'=>__('Ticket Type selection is required'));
		$fields['serialno']  = array('type'=>'text',      'required'=>1, 'error'=>__('Serial No cannot be empty'));
		...
		$serialId = Serial::getIdBySerial($vars['serialno']);
		if($serialId <= 0)
			$serialId = 1;
        ...
        $sql='UPDATE '.TICKET_TABLE.' SET updated=NOW() '
			...
			.' ,serial_id='.db_input($serialId)
			.' ,nature_call='.db_input($vars['natureofcall'])
			.' ,customer_ticket_id='.db_input($vars['custrefno'])
			.' ,vendor_ticket_id='.db_input($vars['venrefno'])
			.' ,call_logged_in='.db_input($vars['calllog'])
   ...
   /*============== Static functions. Use Ticket::function(params); =============nolint*/
    ...
    static function create($vars, &$errors, $origin, $autorespond=true,
        ...
        switch (strtolower($origin)) {
            case 'web':
                $fields['topicId']  = array('type'=>'int',  'required'=>1, 'error'=>__('Select a Ticket Type'));
                break;
            case 'staff':
		...
		function getIdBySerial($serial) {
        $id=0;
        $sql ='SELECT serial_id FROM '.SERIAL_TABLE.' WHERE serial_no='.db_input($serial);
        if(($res=db_query($sql)) && db_num_rows($res))
            list($id)=db_fetch_row($res);
        ...
        $source = $source ?: 'Web';
		$calllog = $vars['calllog'] ?: $ticket->getCallLoggedIn();
		$venrefno = $vars['venrefno'] ?: '';
		$natureofcall = $vars['natureofcall'] ?: 'Warranty';
		$custrefno = $vars['custrefno'] ?: 'No Ref Num';
		$serialCount = 0;
		$serialCount = count($vars['serialno']);
		
		
		$serialId = getIdBySerial($vars['serialno']);
		...
		//if ($serialId <= 0)
		//	$serialId = Serial::create($vars,$errors);
        ...
        $sql='INSERT INTO '.TICKET_TABLE.' SET created=NOW() '
			...
			.' ,call_logged_in='.db_input($calllog)
			.' ,customer_ticket_id='.db_input($custrefno)
			
			.' ,nature_call='.db_input($natureofcall);
    ...
    static function open($vars, &$errors) {
        ...
        if (!$vars['uid']) {
			...
			if (!$vars['serialno'])
                $errors['serialno'] = __('Serial No is required');
			
			if (!$vars['calllog'])
                $errors['calllog'] = __('Ticket Created Location is required');
        }
