...
/**
 * Updates a single database stream. In the classical sense, TesNow only
...
class StreamUpgrader extends SetupWizard {
    ...
    function onError($error) {
        ...
        if($email) {
            ...
            Mailer::sendmail($thisstaff->getEmail(), $subject, $error,
                '"'._S('DQServ Alerts')."\" <{$thisstaff->getEmail()}>");
        }
    ...
    function getNextAction() {
        ...
        $action=sprintf(__('Upgrade TesNow to %s'), $this->getVersion());
