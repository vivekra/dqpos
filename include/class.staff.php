...
class Staff extends AuthenticatedUser {
    ...
    function updateProfile($vars, &$errors) {
        ...
        elseif(($uid=Staff::getIdByEmail($vars['email'])) && $uid!=$this->getId())
            $errors['email']=__('Email already in-use by another staff');
    ...
    function sendResetEmail($template='pwreset-staff') {
        ...
        if ($info['log'])
            $ost->logWarning(_S('Staff Password Reset'), sprintf(
             _S('Password reset was attempted for staff: %1$s<br><br>
    ...
    function save($id, $vars, &$errors) {
        ...
        if($id) {
            ...
            $errors['err']=sprintf(__('Unable to update %s.'), __('this staff'))
            ...
            $errors['err']=sprintf(__('Unable to create %s.'), __('this staff'))
               .' '.__('Internal error occurred');
        }
