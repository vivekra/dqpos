...
var scp_prep = function() {
    ...
    $('.email.typeahead').typeahead({
	 ...
	 /* Typeahead serial no lookup */
    $('.serialno.typeahead').typeahead({
        source: function (typeahead, query) {
            if(query.length > 2) {
                if (last_req) last_req.abort();
                last_req = $.ajax({
        ...
        onselect: function (obj) {
            var fObj=$('.serialno.typeahead').closest('form');
            if(obj.model)
                $('.auto.model', fObj).val(obj.model);
        },
        property: "serialno"
    });
...
$.serialLookup = function (url, cb) {
    $.dialog(url, 201, function (xhr) {
        var serial = $.parseJSON(xhr.responseText);
        if (cb) cb(serial);
    }, {
        onshow: function() { $('#serial-search').focus(); }
    });
};
