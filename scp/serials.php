...
$serial=null;
if($_REQUEST['id'] && !($serial=Serial::lookup($_REQUEST['id'])))
    $errors['err']=sprintf(__('%s: Unknown or invalid ID.'), __('Serial'));

if($_POST){
    switch(strtolower($_POST['do'])){
        case 'update':
            if(!$serial){
                $errors['err']=sprintf(__('%s: Unknown or invalid'), __('serial'));
            }elseif($serial->update($_POST,$errors)){
            ...
            break;
        case 'create':
            if(($id=Serial::create($_POST,$errors))){
                $msg=sprintf(__('Successfully added "%s"'),Format::htmlchars($_POST['serialno']));
                $_REQUEST['a']=null;
            }elseif(!$errors['err']){
            ...
            if(!$_POST['ids'] || !is_array($_POST['ids']) || !count($_POST['ids'])) {
                ...
                switch(strtolower($_POST['a'])) {
                    ...
                    case 'delete':
                        ...
                        if($members)
                            $errors['err']=__('Departments with staffs can not be deleted. Move the agents first.');
                        else {
                            $i=0;
		...
		*/
			case 'import-serials':
            $status = Serial::importFromPost($_FILES['import'] ?: $_POST['pasted']);
