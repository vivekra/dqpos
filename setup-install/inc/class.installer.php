...
class Installer extends SetupWizard {
    ...
    function install($vars) {
        ...
        if($vars['prefix'] && substr($vars['prefix'], -1)!='_')
            $this->errors['prefix']=__('Bad prefix. Must have underscore (_) at the end. e.g \'dq_\'');
        ...
        if(!$this->errors) {
            ...
            elseif(explode('.', db_version()) < explode('.', $this->getMySQLVersion()))
                $this->errors['db']=sprintf(__('TesNow requires MySQL %s or later!'),$this->getMySQLVersion());
        ...
        if(!$this->errors) {
            ...
            $sql='INSERT INTO '.TABLE_PREFIX.'email (`name`,`email`,`created`,`updated`) VALUES '
                    ...
                    .",('DQServ Alerts','alerts@$domain',NOW(),NOW())"
