...
/*********************************************************************
    ...
    Peter Rotich <peter@TesNow.com>
    Copyright (c)  2013-2014 DQSupport   http://www.dqserv.com/   Released under the GNU General Public License WITHOUT ANY WARRANTY.      Derived from TesNow by Peter Rotich <peter@TesNow.com>.    See LICENSE.TXT for details.
...
//define('OSTICKET_CONFIGFILE','../include/ost-config.php'); //TesNow config file full path.
...
$wizard=array();
$wizard['title']=__('TesNow Installer');
$wizard['tagline']=sprintf(__('Installing TesNow %s'),$installer->getVersionVerbose());
$wizard['logo']='logo.png';
$wizard['menu']=array(__('Installation Guide')=>'http://dqserv.com/wiki/Installation',
        __('Get Professional Help')=>'http://dqserv.com/support');
    ...
    switch(strtolower($_POST['s'])) {
        ...
        case 'install':
            if($installer->install($_POST)) {
                ...
                $errors['err']=__('Error installing TesNow - correct the errors below and try again.');
            }
            break;
        case 'subscribe':
            if(!trim($_POST['name']))
    ...
    case 'install':
        if(!$installer->config_exists()) {
            $inc='file-missing.inc.php';
                ...
                || preg_match("/define\('OSTINSTALLED',TRUE\)\;/i",$cFile)) { //TesNow already installed or empty config file?
